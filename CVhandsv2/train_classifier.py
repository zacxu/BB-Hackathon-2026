import pickle

from sklearn.ensemble import RandomForestClassifier
from sklearn.model_selection import train_test_split
from sklearn.metrics import accuracy_score
import numpy as np


data_dict = pickle.load(open('./data.pickle', 'rb'))

# Filter out entries with inconsistent lengths
data_list = data_dict['data']
labels_list = data_dict['labels']

# Find the expected length (should be 42: 21 landmarks * 2 coordinates)
if len(data_list) > 0:
    expected_length = len(data_list[0])
    filtered_data = []
    filtered_labels = []
    
    for i, d in enumerate(data_list):
        if len(d) == expected_length:
            filtered_data.append(d)
            filtered_labels.append(labels_list[i])
        else:
            print(f"Warning: Skipping entry {i} with length {len(d)} (expected {expected_length})")
    
    if len(filtered_data) == 0:
        raise ValueError("No valid data entries found after filtering!")
    
    data = np.asarray(filtered_data)
    labels = np.asarray(filtered_labels)
    
    print(f"Loaded {len(data)} samples with {len(data[0])} features")
else:
    raise ValueError("No data found in pickle file!")

x_train, x_test, y_train, y_test = train_test_split(data, labels, test_size=0.2, shuffle=True, stratify=labels)

model = RandomForestClassifier()

model.fit(x_train, y_train)

y_predict = model.predict(x_test)

score = accuracy_score(y_predict, y_test)

print('{}% of samples were classified correctly !'.format(score * 100))

f = open('model.p', 'wb')
pickle.dump({'model': model}, f)
f.close()
